...
import RolService, { RoleListResponse, RoleResponseDto } from '../../shared/services/rol.service';
...
@Component({
  ...
  imports: [MatTableModule,MatPaginatorModule,MatButtonModule,MatCardModule,MatIconModule,RouterLink],
...
export class PropertyRolComponent implements OnInit {
  ...
  confirmDelete(id: number, name: string): void {
    if (!this.isAdmin()) {
      Swal.fire('Acceso denegado', 'No tienes permisos para eliminar roles.', 'error');
   ...
   deleteRole(roleId: number): void {
    this.service.deleteRole(roleId).subscribe({
      next: () => {
        ...
        this.dataSource.data = this.dataSource.data.filter((role: any) => role.id !== roleId); // Eliminar localmente
      },
      error: () => {
