 ...
 return next(req).pipe(
  catchError((error) => {
    ...
    if (error.status === 401) {
      ...
      Swal.fire('Error','Tu  sesion ha expirado. Por  favor, Inicia sesión nuevamente.','error')
      router.navigate(['/signin']); // Redirige al login
    } else if (error.status === 403) {
      console.error('Error 403: Acceso denegado. Redirigiendo a la página de inicio.');
      // alert('No tienes permiso para acceder a este recurso.');
      Swal.fire('Error','No tienes permiso para acceder a este recurso.','error')
